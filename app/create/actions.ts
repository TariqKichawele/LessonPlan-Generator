    ...
    try {
        ...
        const res = await openaiClient.beta.chat.completions.parse({
            ...
            messages:[
                {
                    role: 'system',
                    content: 'You are a helpful assistant that generates lesson plans for teachers.'
                },
                {
                    role: 'user',
                    content:`
                        Generate a lesson plan for ${topic} with the subtopic of ${subtopic} with a duration of ${duration} minutes for 
                        ${studentLevel} students wuth the objective of ${objective}. The sections of the lesson plan should have a duration 
                        but the sum of all section durations should not exceed ${duration} minutes.
                    `
                }
            ],
        ...
        const lessonPlan = res.choices[0].message.parsed;
        ...
        console.error(error);
        return { success: false, error: "An error occurred while creating the lesson plan." };
    }
}